...
**本文には「なぜそうしたか」を必ず書く。** 特に、試して駄目だった方法や、原因の特定に時間を要した点を残しておくと、後から読む際の助けになる。

```
fix(video): 同じ素材が続くとき映像が止まるのを直す

打楽器のように同じ素材が繰り返し選ばれるパートで、毎回クリップの先頭に戻していたため
静止して見えていた。頭に戻さず再生を続けるようにした。

なお当初はタイムスタンプの問題を疑ったが、連結済みファイルを -c copy で作ると
タイムスタンプが崩れるのは別の不具合であり、こちらは後続のコミットで直している。
```
